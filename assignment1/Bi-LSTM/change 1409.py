...
model.summary()
callback = callbacks.EarlyStopping(monitor='accuracy')
history = model.fit(Ptrain_sent, enc_train_tag, callbacks=[callback], batch_size=128, epochs=15, validation_split=0.2)
...
label = [t for t, i in tindex.items()]
print(classification_report(s,x))
cm = confusion_matrix(s,x,labels = ['NUM', 'ADP', '.', 'PRON', 'VERB', 'NOUN', 'ADJ', 'X', 'CONJ', 'ADV', 'DET', 'PRT', 'PADDING'])

#c = pd.DataFrame(cm)

#file = c.to_table("C:\\Users\\91799\\Pictures\\cse\\NLP\\programs\\pos tagging\\files.csv",index = True)